...
func main() {
	...
	//mailService.Send("ion e-mail test subject", "</h1>outside of context before server's listen!</h1>", to...)

	//inside handler
	app.Get("/send", func(ctx context.Context) {
		content := `<h1>Hello From ion web framework</h1> <br/><br/> <span style="color:blue"> This is the rich message body </span>`

		err := mailService.Send("ion e-mail just t3st subject", content, to...)
